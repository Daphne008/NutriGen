...
export function DietBuilder({
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      void (async () => {
        ...
        const res = await apiClient.get<Food[]>(`/foods${q}`);
  ...
  const predictedScore = useMemo(() => {
    const diff = Math.abs(summaryMetrics.calories - patientReport.data.requiredCalories) / Math.max(patientReport.data.requiredCalories, 1);
    return Math.max(0, Math.round(100 - diff * 70));
  }, [patientReport.data.requiredCalories, summaryMetrics.calories]);

  const suspectedDiagnoses = useMemo(() => {
    const output: string[] = [];
    if (patientReport.data.bmi >= 30) output.push("Obese (BMI > 30)");
    if (patientReport.data.bloodGlucose >= 126) output.push("Diabetic");
    if (patientReport.data.healthRisks.some((risk) => risk.toLowerCase().includes("hyperten"))) output.push("Hypertension");
    if (patientReport.data.age < 18) output.push("Pediatric (< 18 yrs)");
    if (patientReport.data.age >= 65) output.push("Geriatric (>= 65 yrs)");
    if (patientReport.data.healthRisks.some((risk) => risk.toLowerCase().includes("depression"))) output.push("Severe Depression");
    if (patientCategory === "LOW_INCOME") output.push("Below Poverty Line");
    return output;
  }, [patientCategory, patientReport.data.age, patientReport.data.bloodGlucose, patientReport.data.bmi, patientReport.data.healthRisks]);
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-3">
            <p className="text-xs font-medium uppercase tracking-wide text-mutedForeground">
              {loadingFoods ? "Loading foods…" : "Results"}
            </p>
            <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto text-sm">
              ...
              {!loadingFoods && foods.length === 0 && <li className="text-mutedForeground">No foods match your search.</li>}
            </ul>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
            <div className="rounded-md bg-primary/5 p-3 text-sm text-foreground">
              ...
              <p className="mt-1 text-xs text-mutedForeground">
                Select a row below, then use a suggested swap to replace that row&apos;s food.
              </p>
          ...
          <div className="rounded-lg border border-emerald-100 bg-emerald-50/50 p-4">
            <h4 className="text-sm font-semibold text-foreground">Simulation Feedback &amp; Assessment</h4>
            <div className="mt-3 grid gap-4 md:grid-cols-2">
              <div>
                <p className="text-xs font-medium text-mutedForeground">Dietary Assessment</p>
                <ul className="mt-2 space-y-1 text-xs">
                  <li className={predictedScore >= 70 ? "text-emerald-700" : "text-amber-700"}>
                    {predictedScore >= 70
                  ...
                  <li className="text-mutedForeground">
                    Macro Distribution: {patientReport.data.macroTargets.proteinPercent}% / {patientReport.data.macroTargets.carbsPercent}% /{" "}
                    {patientReport.data.macroTargets.fatPercent}%
                  </li>
                </ul>
              </div>
              <div>
                <p className="text-xs font-medium text-mutedForeground">Diagnostic Accuracy</p>
                ...
                <ul className="mt-2 max-h-28 space-y-1 overflow-y-auto text-xs">
                  {suspectedDiagnoses.length === 0 && <li className="text-mutedForeground">No diagnosis misses detected.</li>}
                  {suspectedDiagnoses.map((item) => (
                    <li key={item} className="text-mutedForeground">
                      ⚠ Missed Diagnosis: You failed to flag {item}
                    </li>
                  ))}
                </ul>
              </div>
