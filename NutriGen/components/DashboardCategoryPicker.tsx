  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Choose the clinical themes for this case. Tap a chip to turn it on or off. The categories you pick shape the generated MediGAN patient profile.
        </CardDescription>
      </CardHeader>
        ...
        {selectedCategories.length > 0 && (
          <ul className="space-y-2 rounded-lg border border-border bg-muted/30 p-3 text-sm">
            {selectedCategories.map((c) => {
              const hint = CATEGORY_HOVER[c.id] ?? "";
              return (
                <li key={c.id} className="text-foreground w-fit">
