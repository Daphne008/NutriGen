...
const dietEvaluationService_1 = require("../services/dietEvaluationService");
...
router.post("/diet-plans", async (req, res, next) => {
    try {
        ...
        const foods = await db_1.prisma.food.findMany({ where: { id: { in: foodIds } } });
        const foodById = new Map(foods.map((food) => [food.id, food]));
        const patientReport = payload.patientReportId
            ? await db_1.prisma.patientReport.findFirst({
                where: { id: Number(payload.patientReportId), userId: req.user.id }
            })
            : null;
        const requiredCalories = payload.requiredCalories ??
            Number(patientReport?.data?.requiredCalories ?? 2000);
        const macroTargets = patientReport?.data
            ?.macroTargets;
        ...
        const evaluation = (0, dietEvaluationService_1.evaluateDietPlan)({
            foods: evaluatedFoods,
            requiredCalories,
            macroTargets
        });
        const created = await db_1.prisma.dietPlan.create({
            data: {
                userId: req.user.id,
...
router.get("/diet-plans", async (req, res, next) => {
    try {
        ...
        const plans = await db_1.prisma.dietPlan.findMany({
        ...
        res.json({
            ...
            data: {
                ...
                evaluation: {
                    ...
                    suggestions: (0, dietEvaluationService_1.evaluateDietPlan)({
