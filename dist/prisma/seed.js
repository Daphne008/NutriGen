...
const run = async () => {
    ...
    const patientReport = await prisma.patientReport.create({
        data: {
            ...
            patientCategory: client_1.PatientCategory.DIABETIC,
            data: reportData
        }
    });
    ...
    await prisma.dietPlanItem.createMany({
        data: [
            { dietPlanId: examplePlan.id, foodId: yogurt.id, mealType: client_1.MealType.BREAKFAST, portionMultiplier: 1.5 },
            { dietPlanId: examplePlan.id, foodId: apple.id, mealType: client_1.MealType.BREAKFAST, portionMultiplier: 1 },
            { dietPlanId: examplePlan.id, foodId: chicken.id, mealType: client_1.MealType.LUNCH, portionMultiplier: 1.8 },
            { dietPlanId: examplePlan.id, foodId: rice.id, mealType: client_1.MealType.LUNCH, portionMultiplier: 1.7 },
            { dietPlanId: examplePlan.id, foodId: lentils.id, mealType: client_1.MealType.DINNER, portionMultiplier: 1.6 },
            { dietPlanId: examplePlan.id, foodId: milk.id, mealType: client_1.MealType.SNACK, portionMultiplier: 1.2 }
        ]
    });
